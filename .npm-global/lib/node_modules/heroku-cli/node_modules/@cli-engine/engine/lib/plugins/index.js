...
class Plugins {
    ...
    uninstall(name) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            yield this.init();
            const type = yield this.pluginType(name);
            if (!type) {
                const linked = yield this.link.findByRoot(name);
                if (linked) {
                    name = linked.name;
                }
                else
            ...
            cli_ux_1.default.action.start(`Uninstalling ${name}`);
            if (type === 'user')
                yield this.user.uninstall(name);
            else
                yield this.link.uninstall(name);
    ...
    init() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            yield yield this.plugins.map(p => p.load());
        });
    }
